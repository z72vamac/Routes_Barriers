	...
	\begin{document}
		...
		\begin{enumerate}[label=\textbf{A\arabic*},ref=\textbf{A\arabic*}]
			\item \label{A1}The line segments of $\mathcal B$ are located in general position, i.e., the endpoints of these segments are not aligned. Although it is possible to model the \sout{most general} \CV{aligned} case, one can always slightly modify one of the endpoints so that the segments are \CV{not aligned.} \sout{in general position.}
		...
		\CV{
		These type of elements could be extended further to unions of SOC representable sets introducing binary variables. Each determines in which set is located the point selected to visit the union of these SOC representable sets.
		
		Let $\mathcal U_N=\{C^1_N, \ldots, C^{m_N}_N\}$ be the second order cone representable sets that define the neighborhood $N$. Consider the binary variable $\chi_{N}^{j}$ that is one if $P_N\in C^j_N$, and zero otherwise.
		Hence, for each $N\in \mathcal N$,
		...
		The reader may observe that \eqref{eq:nC} can be replaced by \eqref{eq:uC} without altering the validity of the formulations. However, for the sake of simplicity, only SOC representable sets are considered in the paper.}
...
%		\end{itemize} 
	...
	\CV{
	The \CV{Algorithm \ref{alg:Algorithm3}} describes\CV{, in a general way,} how to construct instances where the neighbourhoods are balls.
...
%		Let $r_{\text{init}}$ be \CV{the} half of the initial length of the barriers. \\
		\CV{Let $R = [LB_x, UB_x]\times[LB_y, UB_y]\subseteq\mathbb R^2$ be the rectangle where centers will be generated.} \\
		Set $points=\{\}$; $\mathcal B=\{\}$; $\mathcal N = \{\}$.}
		% Set $LB=0$, $UB=+\infty$, $\bar z=z^0$.
		Generate $|\mathcal N|$ points uniformly distributed in \CV{$R$} and include them in $points$. \\
		Generate pairwise disjoint barriers that separate  and include them in $\mathcal B$. \\
		Generate neighbourhoods around $points$ and include them in $\mathcal N$. 
	\end{algorithm}
	

	\subsubsection*{Barriers generation}
	In this subsection, we focus on how to generate line segments located in general position without crossings. The idea is to build bisectors that separate any pair of points in $points$. The initial length of each bisector is $r_{\text{init}}$. This length is reduced until it does not intersect any of the previous generated line segments. Algorithm \ref{alg:Algorithm4} reports the pseudocode to generate barriers.
	...
	\begin{algorithm}[H]
		...
		\Input{Let $points$ be those already randomly uniformly generated in $R$. \\
			Let $r_{\text{init}}$ be \CV{the} half of the initial length of the barriers. \\
			Set $\mathcal B=\{\}$.}
			\For{$P, P'\in points$}{
				\If{$\overline{PP'}\cap B=\emptyset,\;\forall B\in\mathcal B$}{
	...
	Once the set of points and barriers are generated, neighbourhoods are created using two different sizes:
	\begin{itemize}
	...
	\end{itemize}
	The first case is used to study the performance of the models \TSPHN \ and \TSPN \ proposed in the paper when neighbourhoods are circles or line segments. The following procedure describes the pseudocode to create circles.
	...
	The second case focuses on the effectiveness of the exact model for the \TSPN \ in terms of the \emph{overlap ratio} of the circles. This ratio, introduced in \cite{mennell_heuristics_2009}, is calculated by dividing the average radius of the neighbourhood sets by the length of the longest side of $R$. \cite{mennell_heuristics_2009} shows that the greater the overlap ratio, the higher the difficulty of the instance. Algorithm \ref{alg:Algorithm5} is slightly modified by setting a fixed value for $radii$, based on instances considered in \cite{}
	...
	\CV{
	In this work, two experiments are designed to study the behaviour of the models \TSPHN \ and \TSPN. In the first experiment, we generate five instances with a number of \CV{randomly-sized} neighbourhoods $|\mathcal N|\in\{5, 10, 20, 30, 50, 60, 65, 70, 75, 80, 100\}$ in $R=[0,100]\times[0, 100]$. These neighbourhoods are balls and line segments are created using the Algorithm \ref{alg:Algorithm5}. For each instance, we run the models with and without strengthening the formulations. The values obtained by Gurobi that are reported in our tables are:

	\begin{itemize}
		\item \boldsymbol{$\#Found$}: number of instances in which the solver can find a solution.
		\item \boldsymbol{$Gap$}: gap between the best incumbent solution with respect to the best bound found by the solver.
		\item \boldsymbol{$Runtime$}: time spent by the solver to obtain the best solution found.
		\item \boldsymbol{$Time_{prepro}$}: time spent by Python to set up the model, including the strengthening of the formulation.
	\end{itemize}

	In the second experiment, based on \cite{behdani_integer-programming-based_2014}, we generate five instances with a number of fixed circles $|\mathcal N|\in\{6, 8, 10, 12, 14, 16, 18, 20\}$. The centers are sampled in $R=[0, 0]\times[16, 10]$. The fixed $radii$ considered are $0.25$, $0.5$, and $1$. Hence, overlap ratios are $0.015625$, $0.03125$ and $0.0625$, respectively. We run \TSPN \ with strengthening to see the performance of our methods when neighbourhoods overlap.
	
	The formulations were coded in Python 3.9.2 (\citet{van_rossum_python_2009}) and solved in Gurobi 9.1.2 (\citet{gurobi_optimization_llc_gurobi_2022}) on an AMD® Epyc 7402p 8-core processor. A time limit of 1 hour was set in the experiments.}
...
\begin{table}[htbp]
	\centering
	\caption{Results for circle instances}
	\label{tab:addlabel}%
	\resizebox{0.5\textwidth}{!}{
	\begin{tabular}{cccccccc}
		\textbf{n\_N} & \textbf{A4} & \textbf{Strength} & \textbf{n\_B} & \textbf{\#Found} & \textbf{Gap} & \textbf{Runtime} & \textbf{Time\_Prepro} \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{5} & \multirow{2}[2]{*}{no} & no & 4,8 & 5 & 0 & 157,24 & 0,29 \bigstrut[t]\\
		&   & yes & 4,8 & 5 & 0 & 1,24 & 0,44 \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 10,4 & 5 & 0,1 & 819,98 & 1,3 \bigstrut[t]\\
		&   & yes & 10,4 & 5 & 0 & 0,61 & 1,16 \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{10} & \multirow{2}[2]{*}{no} & no & 9,2 & 5 & 0,17 & 2193,53 & 2,09 \bigstrut[t]\\
		&   & yes & 9,2 & 5 & 0 & 9,1 & 2,58 \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 19,2 & 5 & 0,17 & 933,67 & 9,59 \bigstrut[t]\\
		&   & yes & 19,2 & 5 & 0 & 2,56 & 6,36 \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{20} & \multirow{2}[2]{*}{no} & no & 17,6 & 5 & 0,17 & 3600 & 16,15 \bigstrut[t]\\
		&   & yes & 17,6 & 5 & 0 & 68,67 & 17,43 \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 35,8 & 5 & 0,21 & 2349,26 & 87,7 \bigstrut[t]\\
		&   & yes & 35,8 & 5 & 0 & 42,45 & 43,49 \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{30} & \multirow{2}[2]{*}{no} & no & 28 & 4 & 0,5 & 3600 & 75,15 \bigstrut[t]\\
		&   & yes & 28 & 5 & 0 & 2034,53 & 65,23 \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 56,4 & 5 & 0,23 & 3027,02 & 512,03 \bigstrut[t]\\
		&   & yes & 56,4 & 5 & 0 & 147,98 & 179,95 \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{50} & \multirow{2}[2]{*}{no} & no & 44,2 & 3 & 0,87 & 3600 & 364,75 \bigstrut[t]\\
		&   & yes & 44,2 & 3 & 0 & 2485,76 & 297,99 \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 89 & 5 & 0,37 & 3600 & 4650,3 \bigstrut[t]\\
		&   & yes & 89 & 5 & 0,1 & 3600 & 1213,85 \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{60} & \multirow{2}[2]{*}{no} & no & 50,2 & 0 & - & - & - \bigstrut[t]\\
		&   & yes & 50,2 & 3 & 0,22 & 3600 & 538,11 \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 100,8 & 5 & 0,15 & 3600 & 8688,65 \bigstrut[t]\\
		&   & yes & 100,8 & 5 & 0,13 & 3600 & 2179,26 \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{65} & \multirow{2}[2]{*}{no} & no & 52,8 & 0 & - & - & - \bigstrut[t]\\
		&   & yes & 52,8 & 2 & 0,35 & 3600 & 671,07 \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 106 & 5 & 0,13 & 3600 & 11250,62 \bigstrut[t]\\
		&   & yes & 106 & 5 & 0,17 & 3600 & 2843,48 \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{70} & \multirow{2}[2]{*}{no} & no & 57,6 & 0 & - & - & - \bigstrut[t]\\
		&   & yes & 57,6 & 3 & 0,12 & 3600 & 898,99 \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 115,6 & 5 & 0,29 & 3600 & 17366,28 \bigstrut[t]\\
		&   & yes & 116,33 & 5 & 0,32 & 3600 & 4106,95 \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{75} & \multirow{2}[2]{*}{no} & no & 63,2 & 0 & - & - & - \bigstrut[t]\\
		&   & yes & 63,2 & 0 & - & - & - \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 126,8 & 4 & 0,24 & 3600 & 26363,48 \bigstrut[t]\\
		&   & yes & 126,8 & 3 & 0,23 & 3600 & 5382,14 \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{80} & \multirow{2}[2]{*}{no} & no & 64 & 0 & - & - & - \bigstrut[t]\\
		&   & yes & 64 & 0 & - & - & - \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 128,6 & 0 & - & - & - \bigstrut[t]\\
		&   & yes & 128,6 & 0 & - & - & - \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{100} & \multirow{2}[2]{*}{no} & no & 81,6 & 0 & - & - & - \bigstrut[t]\\
		&   & yes & 81,6 & 0 & - & - & - \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 163,2 & 0 & - & - & - \bigstrut[t]\\
		&   & yes & 81,6 & 0 & - & - & - \bigstrut[b]\\
		\hline
	\end{tabular}}%
...
\begin{table}[htbp]
	...
	\begin{tabular}{cccccccc}
		\textbf{n\_N} & \textbf{A4} & \textbf{Strength} & \textbf{n\_B} & \textbf{\#Found} & \textbf{Gap} & \textbf{Runtime} & \textbf{Time\_Prepro} \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{5} & \multirow{2}[2]{*}{no} & no & 4,8 & 5 & 0 & 42,59 & 0,91 \bigstrut[t]\\
		&   & yes & 4,8 & 4 & 0 & 0,42 & 1,47 \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 10,4 & 5 & 0 & 22,03 & 1,58 \bigstrut[t]\\
		&   & yes & 10,4 & 5 & 0 & 0,38 & 1,57 \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{10} & \multirow{2}[2]{*}{no} & no & 9,2 & 5 & 0,42 & 2884,22 & 6,6 \bigstrut[t]\\
		&   & yes & 9,2 & 5 & 0 & 1,69 & 9,61 \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 19,2 & 5 & 0,3 & 1448,73 & 11,8 \bigstrut[t]\\
		&   & yes & 19,2 & 5 & 0 & 1,53 & 9,24 \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{20} & \multirow{2}[2]{*}{no} & no & 17,6 & 5 & 0,2 & 3600 & 50,93 \bigstrut[t]\\
		&   & yes & 17,6 & 5 & 0 & 11,66 & 74,89 \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 35,8 & 5 & 0 & 145,06 & 111,34 \bigstrut[t]\\
		&   & yes & 35,8 & 5 & 0 & 8,05 & 64,01 \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{30} & \multirow{2}[2]{*}{no} & no & 28 & 5 & 0,4 & 3600 & 201,05 \bigstrut[t]\\
		&   & yes & 28 & 5 & 0 & 54,35 & 246,96 \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 56,4 & 5 & 0,18 & 1039,08 & 672,46 \bigstrut[t]\\
		&   & yes & 56,4 & 5 & 0 & 96,72 & 270,56 \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{50} & \multirow{2}[2]{*}{no} & no & 44,2 & 4 & 0,3 & 3600 & 960,53 \bigstrut[t]\\
		&   & yes & 44,2 & 5 & 0 & 311,49 & 1043,15 \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 89 & 5 & 0 & 1445,39 & 4654,95 \bigstrut[t]\\
		&   & yes & 89 & 5 & 0 & 353,17 & 1292,12 \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{80} & \multirow{2}[2]{*}{no} & no & 64 & 1 & 0,83 & 3600 & 4205,41 \bigstrut[t]\\
		&   & yes & 64 & 5 & 0 & 1775,16 & 2858,51 \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 128,6 & 5 & 0,11 & 3471,06 & 29073,69 \bigstrut[t]\\
		&   & yes & 128,6 & 5 & 0 & 2701,21 & 7031,87 \bigstrut[b]\\
		\hline
		\multirow{4}[4]{*}{100} & \multirow{2}[2]{*}{no} & no & 81,6 & 0 & - & - & - \bigstrut[t]\\
		&   & yes & 81,6 & 4 & 0 & 1761,08 & 6620,09 \bigstrut[b]\\
		\cline{2-8}      & \multirow{2}[2]{*}{yes} & no & 163,4 & 5 & 0,48 & 3600 & 91153,55 \bigstrut[t]\\
		&   & yes & 163,4 & 5 & 0 & 2720,76 & 19429,1 \bigstrut[b]\\
		\hline
	\end{tabular}%
	\label{tab:addlabel}%
\end{table}%
...
\begin{table}[htbp]
	...
	\begin{tabular}{ccccccc}
		\boldsymbol{$|\mathcal N|$} & \boldsymbol{$|\mathcal B|$} & \boldsymbol{$Radii$} & \boldsymbol{$\#Found$} & \boldsymbol{$Gap$} & \boldsymbol{$Runtime$} & \boldsymbol{$Time_{prepro}$} \bigstrut[b]\\
		\hline
		\multirow{3}[2]{*}{6} & \multirow{3}[2]{*}{8,6} & 0,25 & 10 & 0 & 2,58 & 1,21 \bigstrut[t]\\
		&   & 0,5 & 10 & 0 & 16,31 & 1,26 \\
		&   & 1 & 10 & 0,03 & 1512,68 & 1,4 \bigstrut[b]\\
		\hline
		\multirow{3}[2]{*}{8} & \multirow{3}[2]{*}{12,7} & 0,25 & 10 & 0 & 14,12 & 3,07 \bigstrut[t]\\
		&   & 0,5 & 10 & 0 & 171,44 & 3,16 \\
		&   & 1 & 10 & 0,27 & 3600 & 3,56 \bigstrut[b]\\
		\hline
		\multirow{3}[2]{*}{10} & \multirow{3}[2]{*}{18} & 0,25 & 10 & 0 & 199,23 & 7,35 \bigstrut[t]\\
		&   & 0,5 & 10 & 0,03 & 1453,59 & 7,55 \\
		&   & 1 & 5 & 0,35 & 3600 & 8,5 \bigstrut[b]\\
		\hline
		\multirow{3}[2]{*}{12} & \multirow{3}[2]{*}{19,7} & 0,25 & 10 & 0 & 149,49 & 10,12 \bigstrut[t]\\
		&   & 0,5 & 10 & 0,09 & 3454,33 & 10,54 \\
		&   & 1 & 6 & 0,46 & 3600 & 12,46 \bigstrut[b]\\
		\hline
		\multirow{3}[2]{*}{14} & \multirow{3}[2]{*}{24,5} & 0,25 & 10 & 0 & 374,8 & 23,53 \bigstrut[t]\\
		&   & 0,5 & 10 & 0,11 & 3600 & 24,23 \\
		&   & 1 & 4 & 0,65 & 3600 & 26,56 \bigstrut[b]\\
		\hline
		\multirow{3}[2]{*}{16} & \multirow{3}[2]{*}{29,4} & 0,25 & 10 & 0,01 & 2486,62 & 40,2 \bigstrut[t]\\
		&   & 0,5 & 8 & 0,16 & 3600 & 41,1 \\
		&   & 1 & 0 & - & 3600 & 43.51 \bigstrut[b]\\
		\hline
		\multirow{3}[2]{*}{18} & \multirow{3}[2]{*}{32} & 0,25 & 10 & 0,03 & 3053,15 & 52,18 \bigstrut[t]\\
		&   & 0,5 & 7 & 0,21 & 3600 & 53,05 \\
		&   & 1 & 0 & - & 3600 & 54,03 \bigstrut[b]\\
		\hline
		\multirow{3}[2]{*}{20} & \multirow{3}[2]{*}{37} & 0,25 & 10 & 0,03 & 3040,06 & 75,25 \bigstrut[t]\\
		&   & 0,5 & 6 & 0,32 & 3600 & 80,03 \\
		&   & 1 & 0 & - & 3600 & 86,85 \bigstrut[b]\\
		\hline
	\end{tabular}}%
	\label{tab:addlabel}%
\end{table}%
