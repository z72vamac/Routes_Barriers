...
%	\bigskip
	...
	The major advantage of this approach is that, once the sets $N_S^*$ and $N_T^*$ are calculated beforehand, the entire graph $\GSPPS$ is fixed because the incident edges can be computed for each point $P_S(P_B^i)$ and $P_T(P_B^i)$, $P_B^i\in V_{\mathcal B}$, separately. Figure \ref{fig:dominatingset} shows how the dominating sets $N_S^*$ and $N_T^*$ are calculated for an instance of the \SPPN. \JP{**Parece que faltan dos puntos azules**}
