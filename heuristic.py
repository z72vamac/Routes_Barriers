...
def heuristic(barriers, neighborhoods, prepro=True, log=False, dominant = False, picture=False, time_limit=7200, init=False):
    ...
    if not(dominant):
        ...
        for a, b, c, d, e, f in alpha_index:
            ...
            if a >= 0:
                if c < 0 and e >= 0:
                    if prepro:
                        ...
                        if U <= -1e-6:
                            ...
                            # print('Hola')
                            # print((L, U))
                            model.addConstr((1 - alpha[a, b, c, d, e, f]) * L <= af.determinant(barriers[a][b], [point[c, d, 0], point[c, d, 1]], barriers[e][f]))
                            model.addConstr(af.determinant(barriers[a][b], [point[c, d, 0], point[c, d, 1]], barriers[e][f]) <= U * alpha[a, b, c, d, e, f])
                ...
                if c >= 0 and e < 0:
                    model.addConstr((1 - alpha[a, b, c, d, e, f]) * L <= af.determinant(barriers[a][b], barriers[c][d], [point[e, f, 0], point[e, f, 1]]))
                    model.addConstr(af.determinant(barriers[a][b], barriers[c][d], [point[e, f, 0], point[e, f, 1]]) <= U *alpha[a, b, c, d, e, f])
                if c < 0 and e < 0:
                    model.addConstr((1 - alpha[a, b, c, d, e, f]) * L <= af.determinant(barriers[a][b], [point[c, d, 0], point[c, d, 1]], [point[e, f, 0], point[e, f, 1]]))
                    model.addConstr(af.determinant(barriers[a][b], [point[c, d, 0], point[c, d, 1]], [point[e, f, 0], point[e, f, 1]]) <= U * alpha[a, b, c, d, e, f])
        ...
        for a, b, c, d in epsilon_index:
            model.addConstr(gp.quicksum(delta[a, b, c, d, e, f, g, h] for e, f, g, h in indices_barriers) - len(barriers) + 1 <= epsilon[a, b, c, d])
            model.addConstr(len(barriers) * epsilon[a, b, c, d] <= gp.quicksum(delta[a, b, c, d, e, f, g, h] for e, f, g, h in indices_barriers))
        ...
        for v_n, j in vertices_neighborhood:
            model.addConstr(gp.quicksum(y[v, i, v_n, j] for v, i in vertices_total if (v, i, v_n, j) in edges_neighborhood) >= 1)
        ...
        objective = gp.quicksum(p[index] for index in p.keys()) + gp.quicksum(dist[index] * y[index] for index in edges_barrier)
