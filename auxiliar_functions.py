...
def canseeN(neighborhood1, neighborhood2, barriers):
    ...
    if type(neighborhood1) is neigh.Circle and type(neighborhood2) is neigh.Circle:
        ...
        for mu1 in mus1:
            for mu2 in mus2:
                segment = [center1 + mu1 * nr_u, center2 + mu2 * nr_u]

                cansee = True
                for barrier in barriers:
                    if intersect(segment, barrier):
                        cansee = False
                        break
...
def estima_det(neighborhood, barrera):
    ...
    if type(neighborhood) is neigh.Circle:
        ...
        def equations(p):
            x, y = p
            eq1 = (2*a*(PB2x-PB1x)+c*(PB2y-PB1y))*x+(c*(PB2x-PB1x)+2*b*(PB2y-PB1y))*y+(d*(PB2x-PB1x)+e*(PB2y-PB1y))
            eq2 = a*x**2+b*y**2+c*x*y+d*x+e*y+f

            return (eq1, eq2)

        x_mas, y_mas = fsolve(equations, (0, 0))
        ...
        vector = np.array(center) - np.array([x_mas, y_mas])

        otro_punto = np.array([x_mas, y_mas]) + 2*vector

        x_menos = otro_punto[0]
        y_menos = otro_punto[1]
